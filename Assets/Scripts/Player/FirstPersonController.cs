...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(cameraCanMove)
        {
            yaw += lookInput.x * mouseSensitivity * 0.1f;

            if (!invertCamera)
                pitch -= lookInput.y * mouseSensitivity * 0.1f;
            else
                pitch += lookInput.y * mouseSensitivity * 0.1f;
        ...
        if (enableZoom)
        {
            bool zoomInput = inputActions.Player.Zoom.IsPressed();

            if (!holdToZoom && zoomInput && !isSprinting && !isZoomed)
            {
                isZoomed = true;
            }
            else if (!holdToZoom && !zoomInput && isZoomed)
            {
                isZoomed = false;
            }
            ...
            if (holdToZoom)
            {
                isZoomed = zoomInput && !isSprinting;
            }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.maxLookAngle = EditorGUILayout.Slider(new GUIContent("Max Look Angle", "Determines the max and min angle the player camera is able to look."), fpc.maxLookAngle, 0, 100);
        GUI.enabled = true;
